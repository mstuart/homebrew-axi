...
function withDescription(
  ...
  help.push(
    `Run \`homebrew-axi info ${name} --full\` for the complete description`
  );
}
...
function withInstalls(
  ...
  if (installs30d !== undefined) {
    out.installs30d = installs30d;
  }
  if (installs90d !== undefined) {
    out.installs90d = installs90d;
  }
  if (installs365d !== undefined) {
    out.installs365d = installs365d;
  }
}
...
function formatFormula(
  ...
  if (formula.homepage) {
    out.homepage = formula.homepage;
  }
  if (formula.license) {
    out.license = formula.license;
  }
  if (formula.versions?.stable) {
    out.version = formula.versions.stable;
  }
  ...
  if (depCount > 0) {
    help.push(
      `Run \`homebrew-axi deps ${formula.name}\` to see the dependency breakdown`
    );
  }
  if (help.length > 0) {
    out.help = help;
...
function formatCask(
  ...
  if (cask.name && cask.name.length > 0) {
    out.title = cask.name[0];
  }
  withDescription(out, help, cask.token, cask.desc, full);
  if (cask.homepage) {
    out.homepage = cask.homepage;
  }
  if (cask.version) {
