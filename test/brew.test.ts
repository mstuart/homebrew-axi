...
function respond(error: (Error & { code?: string | number }) | null, stdout: string, stderr: string) {
...
describe("brew.ts", () => {
  ...
  it("refuses to run a subcommand outside the read-only allowlist", async () => {
    ...
    await expect(brewExec(["install", "wget"])).rejects.toThrow(/refuses to run/);
  ...
  it("brewJson parses stdout as JSON on success", async () => {
    ...
    respond(null, JSON.stringify({ formulae: [], casks: [] }), "");
  ...
  it("brewJson throws UNKNOWN on unparsable stdout", async () => {
    ...
    await expect(brewJson(["outdated", "--json=v2"])).rejects.toMatchObject({ code: "UNKNOWN" });
  });
  ...
  it("brewExec throws a mapped error on non-zero exit", async () => {
    ...
    await expect(brewExec(["list", "--versions"])).rejects.toMatchObject({ code: "NOT_FOUND" });
  });
